...
#include "npretype.h"  //8.9
#include "nparamap.h" //181199
/*
typedef struct
{
	double cyan;
...
class _FMI_DLL NFmiTextParamRect : public NFmiParamRect
{
	public:
		NFmiTextParamRect(void)
		  :NFmiParamRect()
		  ,fUseSelectLatinFont(kFalse) //121198
...
*/		
		...
		virtual FmiBoolean		ReadDescription(NFmiString& retString)=0; //5.3
//		virtual NFmiParamRect*	Clone(void) const; 
      int ConvertDefText(NFmiString & object);
		virtual FmiBoolean ReadRemaining(void);  //300699
...
//		void SetColor(float theC, float theM, float theY, float theK); //3.11.00 pois
		void SetTextAttributes(NFmiString theFont      //5.3
							  ...
							  ,NFmiString theStyle
							  ,FmiCMYK theColor
//							  ,FmiBoolean theColorValueDepending = isFalse
								)
		                {itsFont = theFont;
...
//		FmiBoolean GetColorValueDepending(void)const {return fColorValueDependent;};
		virtual FmiBoolean WriteCode(const NFmiString& theText   //12.10
...
//		FmiBoolean IsPureBlack(void);
	...
	private:
		FmiBoolean WriteShowString( double x         //161198
					    ,double y
	...
	protected:                //10.2
		...
		FmiBoolean fUseSelectLatinFont;  //121198		
		NFmiString itsFont;
		FmiDirection itsAlignment;
		NFmiString itsStyle;
//		FmiCMYK itsColor;	//3.11.00 pois k‰ytet‰‰n environmentin geneerist‰		           
//		FmiBoolean fColorValueDependent; //2.11.00 pois, colorMapping hoitaa
		FmiBoolean fParenthesis;         //20.8
