...
class _FMI_DLL NFmiPressText : public NFmiPressScaling
{
	public:
		NFmiPressText(void)
		  ...
		  ,itsTopMargin(-1000.)
		  ,itsIndent (0.)
		  ,fLoopErrorReported(kFalse)
		  ,fInParagraph(0)
		  ,fUpperCase(0)
		  ,fLowerCase(0)
		  ,fAddLocalTime(kFalse)  
		  ,itsText(0)
...
//		void SetColor(float theC, float theM, float theY, float theK);
		virtual FmiBoolean SetText(void){return kTrue;};//131099 tarvitaan NFmiGiventextiä varten
		...
		void SetTextAttributes(NFmiString theFont      
		                ...
		                {itsFont = theFont;
                         ...
                         itsStyle = theStyle;
                         /*itsColor = theColor;*/};
        FmiBoolean WriteString(const NFmiString& commentString,FmiPressOutputMode theOutput);																					

	    virtual FmiBoolean WritePSUpdatingSubText(FmiPressOutputMode theOutput){return WritePS(theOutput);}; //31.8.00

//		FmiBoolean IsPureBlack(void); 
...
//		FmiCMYK GetColor(void)const {return itsColor;}; //3.11.00
	    ...
	    virtual	FmiBoolean      IsInParagraph(void){return fInParagraph;};
	...
	protected:               
		...
		FmiBoolean fLoopErrorReported; //250899
		FmiBoolean fInParagraph;     
		NFmiPoint  itsParagraphMove;  
		NFmiString itsAddInFront;   
		NFmiString itsAddAfter;     
		FmiBoolean fUpperCase;  
		FmiBoolean fLowerCase;   
		FmiBoolean fAddLocalTime;  
...
//		FmiCMYK itsColor;    //3.11.00
		double itsCharSpace;      
//		FmiBoolean fColorValueDependent; ei ole arvoa	  
