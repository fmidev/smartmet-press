...
class _FMI_DLL NFmiPressProduct : public NFmiPressTimeDescription 
{
 ...
 //  NFmiPressProduct(NFmiQueryData* theData); //3.12
	...
	bool WritePS(FmiPressOutputMode=kPostScript); //15.3.00 mod.
...
//	NFmiPressProduct	&operator= (const NFmiPressProduct &thePressProduct);
   ...
   int     ConvertDefText(NFmiString & object);
	bool SetSegmentData(const NFmiString& theDataName); //22.3.00
	bool PreProcessDefinition(const std::string& inFileName, const std::string& outFileName); //25.10.01
   bool PreProcessProduct( std::ifstream& origInput,  std::ofstream& output);    
//   bool CleanFromComments(std::ifstream& inFile);
//	NFmiString FirstDataName(void);                   
	bool SetAllLanguages(FmiLanguage theLanguage); //24.3.00   
	...
	bool SetMaskNumber(unsigned long theNumber); //7.9.01
	bool SetFirstStation(const NFmiLocation& theLocation); //17.3.2000   
	bool SetStationRename(const NFmiRenaming& theRename); //7.6.00 
	bool SetImagePreNames(const NFmiLocation& theLocation); //29.9.00   
	bool SetProductName(const NFmiString& theName);//20.3.00 
	bool SetProductNameFormat(const NFmiString& theName);//15.1.01 
	bool SetLogFile(bool OnOff);
    bool ReadData();  
	bool DefinePar(const NFmiString& theString);  
	bool ReplacePar(NFmiString& theString);  
	bool ReadNameToLonLatList(); //11.8.99
	bool Close(); //17.3.00 
	NFmiString GetInFileName(void)const {return itsInFileName;}; //21.3.00
	bool SetAllTimes(const NFmiMetTime& theTime); //22.3.00   
//	NFmiString StationName(); //8.6.00
	...
	bool SetFirstObjectActivity(bool theActivity);     //30.11.00   
 ...
 private:
	 bool ReadQueryData(NFmiQueryData* theQD 
		                     ,char *fileName);
	 bool ConstructOutFileName(); //16.3.00
...
public:
 ...
 private:
	 ...
	 bool fChangeScandinavian; //17.1.00
	 bool fDataRead;    //050199
