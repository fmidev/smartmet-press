...
class _FMI_DLL NFmiPsSymbol : public NFmiPressScaling 
{
	...
	//						           ,NFmiPoint(1.,1.))
        ...
        virtual FmiBoolean      MakeAndWritePS(NFmiPoint place); //t‰n voi sitten joskus hyl‰t‰
		virtual FmiBoolean      WritePS(FmiPressOutputMode theOutput); //15.3.00
		        NFmiString      GetSymbol(void) const {return itsSymbol;};//190299
//6.3       FmiBoolean              Set( NFmiRectScale &theRectScale
//							   , ofstream theDestinationFile);	

    protected:           
		virtual FmiBoolean CopyShortSymbol2Dest(void); 
        virtual FmiBoolean ConvertOrig2Short(void);
