...
#include "nrectsca.h"
#include "nprtides.h"  
#include "nPsWrite.h"  //15.3.00
#include "NFmiMultiParamMapping.h"  //23.9.99

class _FMI_DLL NFmiPressParam;

extern FmiBoolean NFmiCopyFile(std::ifstream&, std::ofstream&);
extern FmiBoolean NFmiWritePSConcat(NFmiRectScale, std::ofstream&);
extern FmiBoolean NFmiWritePSConcatRotating(NFmiRectScale, float, std::ofstream&); //LW 20.11
extern FmiBoolean NFmiWritePSEnd(std::ofstream&);
	...
	public:
		NFmiParamRect(void)
		  : NFmiRect()  
		  {
		    ...
		    fLevelErrorReported=kFalse;  //5.5.00
...
*/
		FmiBoolean SetPressParam (NFmiPressParam* pressParam) 
								{itsPressParam = pressParam;
								return kTrue;};
		virtual FmiBoolean	Set(NFmiDataIdent theParam, NFmiRect theRect);
		FmiBoolean	SetIdent(NFmiDataIdent theParam); 
		FmiBoolean	SetRect(NFmiRect theRect);        
		...
		FmiBoolean      IsNewScaling(void) {return fNewScaling;}; 
		void            SetNewScaling(FmiBoolean newScaling) {fNewScaling=newScaling;}; 
...
//		float Celsius2Fahrenheit(float value);
	...
	protected:
		FmiBoolean SetRelModifierTimes(long startRelHour, long endRelHour);
		long RelativeHour(void)const;             
		FmiBoolean SetRelativeHour(NFmiFastQueryInfo* data, const NFmiString& calledFrom); 
		NFmiMetTime CalculatePeriodTime(long theHour); 
		FmiBoolean SetStationLocalTime(NFmiFastQueryInfo* theQI); //15.5.02 
//		FmiBoolean ResetFromLocalTime(NFmiFastQueryInfo* theQI); 
		virtual FmiInterpolationMethod InterpolationMethod(NFmiFastQueryInfo* theQueryInfo);   
		FmiBoolean ReadCurrentValueArray(NFmiFastQueryInfo* theQueryInfo); 
		FmiBoolean ReadCurrentValue(NFmiFastQueryInfo* theQueryInfo, float& value,FmiBoolean localTimeSet=kFalse); 
		FmiBoolean FloatValue(NFmiFastQueryInfo* theQueryInfo, float& value); 
        virtual FmiBoolean PointOnParam (NFmiFastQueryInfo* theQI, NFmiParam* theParam); //+toka par., 2.5.00
		virtual FmiBoolean PointOnMultiParam(NFmiFastQueryInfo* theQI, short theNum);//23.9.99 
		virtual FmiBoolean PointOnLevel(NFmiFastQueryInfo* theQI);  
...
//		FmiLoopActivity itsLoopActivity; //26.9.01
        ...
        FmiBoolean    fParamErrorReported;  
        FmiBoolean    fLevelErrorReported;  
