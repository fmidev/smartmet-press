...
//	virtual std::istream& Read(std::istream &file);
	...
	NFmiScale& operator+= (const NFmiScale& anOtherScale);
 	NFmiScale& operator-= (const NFmiScale& anOtherScale);
	  ...
	  FmiBoolean Inside         (float theValue) const;
    ...
    virtual void Expand         (float theAdditionToBothEnds=1.);
			void LimitCheck (FmiBoolean theCheck) {itsLimitCheck=theCheck;};
    virtual void ExcludePositive (void);    //2.5.01
    virtual void ExcludeNegative (void);    //2.5.01
      FmiBoolean Ok         (void)const {return itsDataOk;};
...
//			float Difference (void) const;
			 void Check      (void);
	   FmiBoolean IsMissing	 (void) const;

		float itsStartValue;
		float itsEndValue;
		float itsEpsilon;
 FmiBoolean itsDataOk;
 FmiBoolean itsLimitCheck;
