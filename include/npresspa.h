...
//	void    SetDataName(const NFmiString& name){itsDataName = name;};//4.9 takaisin
	FmiBoolean SetData(const NFmiString& dataName);    
//	FmiBoolean SetMaskFile(const NFmiString& dataName);    
	...
	FmiBoolean SetFirstStation(const NFmiLocation& theLocation);    
	FmiBoolean SetStationRename(const NFmiRenaming& theRenaming);    
	FmiBoolean SetAllTimes(const NFmiMetTime& theTime);    
	FmiBoolean SetAllLanguages(FmiLanguage theLanguage);    

	FmiBoolean IsStationLocalTimeMode (void) const{return fStationsAreLocalTime;}; 
	FmiBoolean IsFirstStation (void) const{return fIsFirstStation;}; 
	void SetErrorReported(unsigned short ind){fErrorReported[ind-1] = kTrue;}; 
	FmiBoolean GetErrorReported(unsigned short ind){return fErrorReported[ind-1];}; 
...
protected:
	void UnsetAllErrorReported(void){fErrorReported[0] = kFalse;
									fErrorReported[1] = kFalse;
									fErrorReported[2] = kFalse;
									fErrorReported[3] = kFalse;}; //3.3.00

	 void NextStation(void);
	 FmiBoolean SetLonLat(NFmiStationPoint& theStation); // 16.8.99 NFmiStation-> NFmiStationPoint

 private:
     FmiBoolean FindQDStationName (const NFmiStationPoint& theStation); 
	 bool CreateAreaMask(void); //13.9.01
	 bool CreateStationFromAreaMask(void); //13.9.01

	 FmiBoolean fDataNotNeeded; //3.4.01
	 FmiBoolean fErrorReported[4];  
	 FmiBoolean fIsFirstStation; 
	 FmiBoolean fStationsAreLocalTime;  
	 FmiBoolean fGridMode;         
	 NFmiStation itsCurrentStation; 
	 NFmiString itsName;       
	 FmiBoolean fLonLatMode;   
