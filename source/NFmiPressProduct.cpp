...
bool NFmiPressProduct::AddSubstituteMappingValue(float theOldValue, float theNewValue)
{
...
bool NFmiPressProduct::FindLonLatFromList(NFmiString &theStationName, NFmiPoint &theLonLat)
{
  ...
  {
    if (theStationName != NFmiString("Tyhj‰") && theStationName != NFmiString("None"))
      *itsLogFile << "  WARNING: " << static_cast<char *>(theStationName)
                  << " is not in the name/longlat file" << endl;
    return false;
  }
}
// ----------------------------------------------------------------------
...
unsigned long NFmiPressProduct::FindWmoFromList(const NFmiString &theStationName)
{
  ...
  *itsLogFile << "WMO=" << wmo << endl;  // bbbbb
  ...
  {
    *itsLogFile << "  WARNING: " << static_cast<char *>(theStationName)
                << " does not have a WMO-number in the station name table" << endl;
  }
  return wmo;
}
// ----------------------------------------------------------------------
    ...
    else
    {
      ...
      if (fmiStr1 == "wintertime" || fmiStr1 == "talviaika")
      {
        if (boolGiven && !undef)
        {
          ...
          *itsLogFile << "  Forced season status: Wintertime " << static_cast<char *>(statusString)
                      << endl;
        }
      }
      else if (fmiShortStr1 == "summ" || fmiShortStr1 == "kes‰")
      {
        if (boolGiven && !undef)
        {
          ...
          *itsLogFile << "  Forced season status: Summer " << static_cast<char *>(statusString)
                      << endl;
        }
      }
      else if (fmiShortStr1 == "poll" || fmiShortStr1 == "siit")
      {
        if (boolGiven && !undef)
        {
          ...
          *itsLogFile << "  Forced season status: Pollen Period "
                      << static_cast<char *>(statusString) << endl;
        }
      }
      else if (fmiShortStr1 == "snow" || fmiShortStr1 == "lumi")
      {
        if (boolGiven && !undef)
        {
          ...
          *itsLogFile << "  Forced season status: Snow Period " << static_cast<char *>(statusString)
                      << endl;
        }
      }
      ...
      else if (fmiShortStr1 == "day_" || fmiShortStr1 == "date" ||
               fmiShortStr1 == "p‰iv")  // day:hin alaviiva!!
      {
        if (!undef)
        {
      ...
      else if (fmiShortStr1 == "edit")
      {
        if (boolGiven && !undef)
        {
          ...
          *itsLogFile << "  Forced data (obsolete on Linux): editor "
                      << static_cast<char *>(statusString) << endl;
          if (ownComputer)
          {
      ...
      else if (fmiShortStr1 == "sall" || fmiShortStr1 == "allo")
      {
        if (boolGiven && !undef)
        {
          ...
          *itsLogFile << "  Forced status: allowRandom " << static_cast<char *>(statusString)
                      << endl;
        }
      }
      /*
                                ...
                                if(!undef)
                                      {
                                        ...
                                        *itsLogFile << "  Kausitilanne pakotettu: iltap‰iv‰ alkaa "
         << givenHour << endl;
                                      }
                              }
                              */
      else if (fmiShortStr1 == "muut" || fmiShortStr1 == "vari")
      {
        ...
        {
          string msg =
              string("Invalid variable line in seasonstatus: ") + NFmiValueString(line).CharPtr();
          errors.push_back(msg);
          *itsLogFile << "***ERROR: " << msg << endl;
          continue;
        }
...
// Set the paths right
  ...
  if (!endFile)
  {
    cout << "** Home directory " << GetHome().GetCharPtr() << " missing**" << endl;
...
// Get rid of the Muut directory in Linux
  ...
  *itsLogFile << "Home dir " << static_cast<char *>(origHome) << ": "
              << static_cast<char *>(GetHome()) << endl;
...
bool NFmiPressProduct::ReadQueryData(NFmiQueryData *theQD, char *fileName)
{
  ...
  if (NFmiFileSystem::DirectoryExists(filename))
  {
    ...
    *itsLogFile << "  In the directory '" << filename << "' the newest file is '" << file << "'"
                << endl;
    filename += kFmiDirectorySeparator;
    filename += file;
  }
  ...
  *itsDescriptionFile >> itsObject;
  ...
  while (itsIntObject != 9999 || itsCommentLevel)
  {
    ...
    switch (itsIntObject)
    {
      ...
      case dDataFile:
      {
        ...
        if (fDataRead)
        {
          string msg = string("Data ") + static_cast<char *>(itsDataFileName) +
                       "should be given before 'UseDataStartTime' and 'HoursFromData' and all "
                       "subelements. INTERRUPTED.";
          errors.push_back(msg);
          *itsLogFile << "*** ERROR: " << msg << endl;
          return false;
        }
        ...
        NFmiQueryData *newData = new NFmiQueryData();
        NFmiNamedQueryData *newNData =
            new NFmiNamedQueryData(newData,
                                   itsDataFileName,
      ...
      case dPageSize:  // Pysty/Vaaka nimitys
      {
        ...
        *itsDescriptionFile >> itsObject;
        ...
        if (!uniBBset)
        {
          ...
          if (helpString == NFmiString("a4m") || helpString == NFmiString("a4v"))
            itsPageSize = kA4Maisema;
          else if (helpString == NFmiString("a3m") || helpString == NFmiString("a3v"))
      ...
      case dWeekdayDir:  // direktiivitieto, ei virkaa t‰‰ll‰ mutta veisi aikaa poistaa
                         // commentstripperill‰
      {
  ...
  if (itsLogFile)
  {
    *itsLogFile << "Confirming reaching End-of-Product (should be #END): "
                << static_cast<char *>(itsString) << endl;
    if (itsString.GetLen() >= 2 && NFmiString(itsString.GetCharsPtr(1, 2)) != NFmiString("#E") &&
...
int NFmiPressProduct::ConvertDefText(NFmiString &object)
{
  ...
  if (lowChar == NFmiString("product") || lowChar == NFmiString("productname") ||
  ...
  else if (lowChar == NFmiString("date") || lowChar == NFmiString("p‰iv‰m‰‰r‰"))
    return dDate;
  ...
  else if (lowChar == NFmiString("outdirectory") || lowChar == NFmiString("outdir") ||
           lowChar == NFmiString("tulostus"))
    return dOutDir;
  else if (lowChar == NFmiString("pagesize") || lowChar == NFmiString("arkki"))
    return dPageSize;
  else if (lowChar == NFmiString("palette") || lowChar == NFmiString("paletti"))
    return dPalette;
  else if (lowChar == NFmiString("mapfile") || lowChar == NFmiString("background") ||
           lowChar == NFmiString("pohja"))
    return dMapFile;
  else if (lowChar == NFmiString("datafile") || lowChar == NFmiString("data"))
    return dDataFile;
  else if (lowChar == NFmiString("yeardata") || lowChar == NFmiString("yeardatafile") ||
           lowChar == NFmiString("vuosidata"))
    return dYearDataFile;
  else if (lowChar == NFmiString("newestdatafile") || lowChar == NFmiString("uusindata"))
  ...
  else if (lowChar == NFmiString("mandatorydata") || lowChar == NFmiString("mandatorydatafile") ||
           lowChar == NFmiString("pakollinendata"))
    return dMandatoryDataFile;
  else if (lowChar == NFmiString("maskfile") || lowChar == NFmiString("maskidata"))
    return dMaskFile;
  else if (lowChar == NFmiString("#fixedsymbol") || lowChar == NFmiString("#constimage") ||
           lowChar == NFmiString("#vakiosymboli"))
    return dImageObject;
  else if (lowChar == NFmiString("#subimage") || lowChar == NFmiString("#subimages") ||
           lowChar == NFmiString("#osakuva") || lowChar == NFmiString("#osakuvia"))
    return dSubImage;
  else if (lowChar == NFmiString("#text") || lowChar == NFmiString("#vakioteksti") ||
           lowChar == NFmiString("#teksti"))
    return dTextObject;
  else if (lowChar == NFmiString("#columntext") || lowChar == NFmiString("#palstateksti"))
  ...
  else if (lowChar == NFmiString("#analysistime") || lowChar == NFmiString("#analyysiaika"))
  ...
  else if (lowChar == NFmiString("gifrectangle") || lowChar == NFmiString("gifrajaus"))
    return dBoundingBorder;
  else if (lowChar == NFmiString("boundingbox") || lowChar == NFmiString("boundingborder") ||
           lowChar == NFmiString("aluerajaus"))
    return dUniBoundingBorder;
  else if (lowChar == NFmiString("mapisreduced") || lowChar == NFmiString("pohjaonriisuttu") ||
           lowChar == NFmiString("pohjaonps"))
    return dMapIsReduced;
  else if (lowChar == NFmiString("subviews") || lowChar == NFmiString("lukum‰‰r‰"))
    return dSubViews;
  else if (lowChar == NFmiString("margin") || lowChar == NFmiString("reunus"))
    return dMargin;
  else if (lowChar == NFmiString("gap") || lowChar == NFmiString("v‰li"))
    return dGap;
  else if (lowChar == NFmiString("#pressparam") || lowChar == NFmiString("#segment") ||
    ...
    return dPressParam;
           ...
           lowChar == NFmiString("#constplaces") || lowChar == NFmiString("#vakiosymbolipaikat") ||
  ...
  else if (lowChar == NFmiString("#weekday") || lowChar == NFmiString("#viikonp‰iv‰"))
    return dWeekdayDir;
  ...
  else if (lowChar == NFmiString("cvversion") || lowChar == NFmiString("cvversio"))
    return dCVversion;
  else if (lowChar == NFmiString("encoding") || lowChar == NFmiString("enkoodaus"))
    return dEncoding;
  else
...
// Get rid of the Muut directory in Linux
  ...
  if (!outFile)
  {
    string msg = (string("Output-tiedost ei onnistu: ") + static_cast<char *>(itsOutFile) +
  ...
  while (itsCurrentMap <= itsNumberOfMaps)  // itsNumberOfMaps=koko tuote monistettu n kpl
  {
      ...
      else
      {
        ...
        else
        {
          if (fMapIsEps)
          {
            ...
            else  // Epsiin voi laittaa nolla-BoundaryBording koska alussa on oikea ja maximi
                  // otetaan
                if (itsPalette)  // ei k‰ytˆss‰
              NFmiCopyFileCroppingAndColoring(
                  mapFile, outFile, NFmiRect(NFmiPoint(0., 0.), NFmiPoint(0., 0.)), itsPalette);
            else
              NFmiCopyFileCropping(
                  mapFile, outFile, NFmiRect(NFmiPoint(0., 0.), NFmiPoint(0., 0.)));
          ...
          if (itsLogFile)
            *itsLogFile << "Background copied: " << static_cast<char *>(fullEpsFileName) << endl;
        }
      }
      epsFileName = static_cast<NFmiString *>(epsIter.Next());
      mapFile.close();
    ...
    if (output == kMetaLanguage && itsNumOfWritePS < 2)
    {
      ...
      {
        outFile << endl
                << "New " << itsBoundingBorder.Size().X() << " " << itsBoundingBorder.Size().Y()
                << " Transparent" << endl;
      }
    ...
    //	  if(itsCurrentDataIter) // kaatuu ilman dataa; 2.03 ei en‰‰ kaadukaan
    {
      ...
      while (param)
      {
        ...
        {
          if (itsLogFile)
            *itsLogFile << segNum << ". Segment (" << static_cast<char *>(param->MakeLogComment())
                        << ")" << endl;
  ...
  {
    ConstructOutFileName();
    outFile << endl << "Save " << static_cast<char *>(itsOutFile.Header()) << ".gif" << endl;
  }
...
bool NFmiPressProduct::WriteMetaInit(void)
{
  if (itsNumOfWritePS < 2)
  {
    ...
    outFile << "Draw primitive=>polygon points=>\"7,7 0,20 -7,7\" stroke=>black fill=>$fill "
               "linewidth=>1.5 translate=>\"$x,$y\" rotate=>$rotate"
            << endl;
    outFile << "Draw primitive=>circle points=>\"0,0 8,0\" stroke=>black fill=>$fill "
               "linewidth=>1.5 translate=>\"$x,$y\""
            << endl;
    ...
    if (itsMagicSavePath.IsValue())
      outFile << "SavePath /var/www/share/saakartat/" << static_cast<char *>(itsMagicSavePath)
              << endl
              << endl;
    else
...
// kutsutaan kahteen kertaan: ennen ja j‰lkeen segmenttien
...
#else  // _MSC_VER en tied‰ mist‰ t‰m‰ precedingElementMissing -muuttuja on winkkarilla m‰‰r‰tty,
       // joten otan muuttujan k‰yttˆˆn t‰ss‰
  ...
  while (object)
  {
    ...
    if (object->IsActive() && object->IsToWriteLast() != theDoPreSegments)
    {
      ...
      if (!object->GetOnlyForMissingPrecedingElementFlag() || precedingElementMissing)
      {
...
bool NFmiPressProduct::ConstructOutFileName(void)
{
  ...
  if (!itsProductNameFormat.IsValue())
  {
    ...
    else if (fDataTimeStamp)
    {
      ...
      if (timeFormat.IsValue())
        ...
        timeStampT = NFmiMetTime(itsFirstPlotTime.LocalTime(25.), 1)
                         .ToStr(kYYYYMMDDHH);  // yhteensopivuus meneeseen
      else
  ...
  else
  {
    ...
    while (itsProductNameFormat.NextSubString(NFmiString("/"), subString))
    {
      ...
      if (lowerString == NFmiString("asema") || lowerString == NFmiString("station"))
      {
      ...
      else if (lowerString == NFmiString("ase") || lowerString == NFmiString("sta"))
      {
        addString = FirstSegmentName();
      ...
      else if (lowerString == NFmiString("aikaleima") || lowerString == NFmiString("timestamp"))
      {
        NFmiMetTime computerTime = NFmiMetTime(1);
        NFmiMetTime localTime(computerTime.LocalTime(25.f), 1);
      ...
      else if (lowerString == NFmiString("kie") || lowerString == NFmiString("lan"))
      {
      ...
      else if (lowerString == NFmiString("kieli") || lowerString == NFmiString("language"))
      {
        if (itsLanguage == kFinnish)
...
{
  double oneWidth = (itsPlottingRect.Width() - 2. * itsMargin - (itsNumberOfMaps - 1) * itsMapGap) /
                    itsNumberOfMaps;
  double oneHeight = itsPlottingRect.Height() - 2. * itsMargin;
  double xOffset =
      itsPlottingRect.Left() + itsMargin + (itsCurrentMap - 1) * (itsMapGap + oneWidth);
  double yOffset = itsPlottingRect.Top() + itsMargin;  // pit‰isi olla Bottom()
  itsScale.SetEndScales(
      NFmiRect(NFmiPoint(xOffset, yOffset), NFmiPoint(xOffset + oneWidth, yOffset + oneHeight)));
}
