...
bool NFmiPressProduct::SetFirstSegmentActivity(bool theActivity)
{
  ...
  {
	string msg = "Elementtej‰ puuttuu (de)aktivoitavaksi managerista";
	errors.push_back(msg);
	*itsLogFile << "*** ERROR: " << msg << endl;
	return false;
  }
...
bool NFmiPressProduct::ChangeFirstPossibleObject(bool toNewActivity)
{
  ...
  string msg = "Ei ole elementtej‰ (de)aktivoitavaksi managerista";
  errors.push_back(msg);
  *itsLogFile << "*** ERROR: " << msg << endl;
  return false;
}
...
bool NFmiPressProduct::ReadNameToLonLatList(void)
{
  ...
  if (itsNameToLonLat->AddFile(fileName1, true, true))
	...
	{
	  string msg = "AsemaNimetLonLatUusi.txt:n luku ep‰onnistui";
	  errors.push_back(msg);
	  *itsLogFile << "*** ERROR: " << msg << endl;
	  return false;
	}
  ...
  if(itsNameToLonLat->Empty())
	{
	  if(!ReadNameToLonLatList())
		{
		  string msg = "Nimi/lonlat-tiedoston luku ep‰onnistui";
		  errors.push_back(msg);
		  *itsLogFile << "*** ERROR: " << msg << endl;
		  return false;
		}
  ...
  while(getline(in,line))
	{
	  ...
	  if(end2 == string::npos)
	  ...
	  else
		{
		  ...
		  else if(fmiShortStr1 == "date" || fmiShortStr1 == "p‰iv") //day vain 3 merkki‰!!
			{
			  if(!undef)
				{
...
//*******
   ...
   *itsLogFile << "program version = Release 27.5.2010" << endl;       
...
*/
  ...
  NFmiTime time;
	   *itsLogFile << "  JƒLKEEN READ " << time << " **" << endl;
...
bool NFmiPressProduct::ReadDescription(NFmiString & retString)
{
  ...
  *itsDescriptionFile >> itsObject;
			...
			if(fDataRead)
			  {
				string msg = string("Data ")
				       + static_cast<char *>(itsDataFileName)
   				       + "pit‰‰ antaa ennen 'K‰yt‰DatanAlkuAikaa' ja 'SuhteellinenTuntiDatasta' sek‰ kaikkia j‰senelementtej‰. KESKEYTETƒƒN.";
				errors.push_back(msg);
				*itsLogFile << "*** ERROR: " << msg << endl;
				return false;
			  }
		...
		case dRelativeDataTime:
		  {
			...
			if(firstData)
			  ...
			  {
				string msg = "Pit‰‰ antaa datat ennen 'SuhteellinenTuntiDatasta'. KESKEYTETƒƒN";
				errors.push_back(msg);
				*itsLogFile << "*** ERROR: " << msg << endl;
				return false;
			  }
  ...
  if(itsLogFile)
	{
	  *itsLogFile << "Lopetusrivivarmistus (pit‰‰ olla #LOPPU): "
