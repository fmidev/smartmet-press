...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiCopyFile(ifstream& inFile, ofstream& outFile)
{
...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiCopyFileWithoutShowpage(ifstream& inFile, ofstream& outFile)
...
FmiBoolean NFmiCopyFileColoring(ifstream& inFile, ofstream& outFile, NFmiCmykColorBag*)
{
  const short lineSize = 2800;//130  1.6 Metview:n ps-driveri tuottaa 513 pitki‰ rivi‰
  char inBuf[lineSize];           //  vaikka standardi puhuu 256:sta
  short nColor;
...
//10.6  //18.6 t‰m‰ funktio p‰‰tetty tehd‰ Perlill‰ joten toistaiseksi hyˆdytˆn
        // eik‰ lopullisesti testattu
FmiBoolean NFmiCopyFileCroppingAndColoring(ifstream& inFile, ofstream& outFile
...
//23.11.01--------------------------------------------------------------------------------------
FmiBoolean NFmiWriteBoundingBox(ofstream& outFile
								,NFmiRect theRect)
{ 
	outFile.put('\x0A');		  
	NFmiString newBounding("%%BoundingBox: ");
...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiCopyFileCropping(ifstream& inFile, ofstream& outFile
								,NFmiRect theRect)
{ 
  const short lineSize = 2800; //14.8  //1.6 Metview:n ps-driveri tuottaa 513 pitki‰ rivej‰	
//  unsigned char inBuf[lineSize];
...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiWritePSConcat(NFmiRectScale theScale, ofstream& outFile)
{
...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiWriteEPSConcat(NFmiRectScale theScale, ofstream& outFile)
{
	   double xScale = theScale.GetXScaling();
	   double yScale = theScale.GetYScaling();
...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiWriteEPSConcatClipping(NFmiRectScale theScale, ofstream& outFile, NFmiRect theClippingRect)
{//2.12
	   double xScale = theScale.GetXScaling();
	   double yScale = theScale.GetYScaling();
		double xTrans = theScale.GetEndStartPoint().X() / xScale - theScale.GetStartStartPoint().X(); //2.12
...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiWritePSConcatRotating(NFmiRectScale theScale, float theDirection, ofstream& outFile)
{
...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiWritePSEnd(ofstream& outFile)
{	  
		// Mika: Perkele
		// ostream saveCout = cout;
		// cout = outFile;
...
//--------------------------------------------------------------------------------------
FmiBoolean NFmiWriteEPSEnd(ofstream& outFile)
{	  
		// Mika: Lasse hirteen!
		// ostream saveCout = cout;
		// cout = outFile;
