...
bool NFmiPressComputerTimeText::SetText(void)
{
  ...
  if (itsStringNameTimeFormat.IsValue())
    SetText(
        (static_cast<NFmiPressTime>(useTime)).InterpretToStr(itsStringNameTimeFormat, itsLanguage));
  else
...
int NFmiPressComputerTimeText::ConvertDefText(NFmiString& object)
{
  ...
  else if (lowChar == NFmiString("hourdifference") || lowChar == NFmiString("tuntiero"))
