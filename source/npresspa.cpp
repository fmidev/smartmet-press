...
//17.3.2000----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::SetFirstStation(const NFmiLocation& theLocation)   
{
...
//17.3.2000----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::SetStationRename(const NFmiRenaming& theRenaming)   
{
	if(!itsStations.IsLocations())
		return kFalse;
...
//----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::SetData(const NFmiString& dataName)   
...
//17.3.00----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::SetAllTimes(const NFmiMetTime& theTime)   
{
	SetTime(theTime);
	FmiBoolean retCode;
...
//17.3.00----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::SetAllLanguages(FmiLanguage theLanguage)   
{
...
//----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::ReadDescription(NFmiString& retString) 
{
	...
	FmiBoolean symbolGroupCalled = kFalse;    //8.5.00 
	FmiBoolean helpBool;
	FmiBoolean firstStation = kTrue;
	fIsTimeLoop = false;
	fIsLevelLoop = false;
	...
	FmiBoolean timeOrLevelTableSet = kFalse;
	long statNum;
	double lat, lon;
	...
	*itsDescriptionFile >> itsObject;
	...
	FmiBoolean dataTimeGiven = kFalse;
...
//				text->ScalePlotting();
		...
		case dSymbolGroup:
		{
			...
			else
			{
				...
				// FmiBoolean ok = itsSymbols.ReadDescription(itsString);
...
//			ReadNext();
		...
		case dStationNameReplace: //051198 stationiin verrattuna lisäykset: *** PrintName
		{
			 lon = 0.; //8.2.2000
			 lat = 0.;
			FmiBoolean hasOwnPoint = kFalse;  //4.3 talukkoEscapea varten
			...
			*itsDescriptionFile >> itsObject;    //2.3
			...
			{
				itsString = valueString;
				if(numOfTableStations < 1) helpBool = kFalse;
			}
...
//----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::WritePS(
				 ...
				 ,FmiPressOutputMode theOutput)
{
	...
	if (!itsDataIter)  
	{
		*itsLogFile << "  *** ERROR: data lost: " << (char*)itsDataName << endl;
		return kTrue; //false lopettaa kaikki segmentit
	}
...
//	   if(kTrue) // 8.2.2000:Segmentin ajan ei tarvi enää osua datan aikoihin, mahdollistaa max/min ym 
 	   {
...
//----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::FindQDStationName(const NFmiStationPoint& theStation)
{                                            
	if(!itsDataIter)    
		return kFalse;
...
//----------------------------------------------------------------------------
FmiBoolean NFmiPressParam::SetLonLat(NFmiStationPoint& theStation)
{
