...
int NFmiScalingParamRect::ConvertDefText(NFmiString & object)
{
  ...
  if(lowChar==NFmiString("heightscale") ||
	 lowChar==NFmiString("korkeusskaala"))
	return dHeightScale;

  else if(lowChar==NFmiString("widthscale") ||
		  lowChar==NFmiString("leveysskaala"))
	return dWidthScale;

  else if(lowChar==NFmiString("scalingparameter") ||
		  lowChar==NFmiString("skaalaavaparametri"))
	return dSecondPar;
