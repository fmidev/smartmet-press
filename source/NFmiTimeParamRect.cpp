                                ...
                                FmiPressOutputMode theOutput)
{
  ...
  {
    long wmo = itsPressParam->GetPressProduct()->itsNameToLonLat->FindWmo(
        tempStationPoint.Station()->GetName());
  ...
  if (itsPressParam->IsStationLocalTimeMode())
  {
    ...
    if (IsMissingLonLat())  // eli taulukostakaan ei löytynyt
      *itsLogFile << "*** ERROR: longitudu missing from "
                  << static_cast<char *>(itsStationPoint.GetName()) << "to local time" << endl;
