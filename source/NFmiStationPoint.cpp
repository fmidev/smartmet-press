...
//           4=     - " -                       , käytetty longitudia
{
  ...
  {
    bool isFinland = GetLongitude() > 19.1 && GetLongitude() < 31.6 && GetLatitude() > 59.5 &&
                     GetLatitude() < 70.15;
    if (isFinland)
...
float NFmiStationPoint::GetWmoDifference(void) const
{
  ...
  else if (wmoCode >= 4000 && wmoCode < 4100)  // Islanti
  {
    ...
    useSummerTime = false;
           ...
           (wmoCode >= 6000 &&
            wmoCode < 8500) ||  // Tanska,Hollanti,Belgia,Luxemburg,Sveitsi,Ranska,Espanja,
           (wmoCode >= 9000 &&
            wmoCode < 13700) ||  // Saksa,Itävalta,Tsekki,Slovakia,Puola,Unkari,Jugot,Albania
