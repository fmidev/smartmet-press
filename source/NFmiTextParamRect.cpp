...
NFmiTextParamRect::NFmiTextParamRect(const NFmiTextParamRect &theTextParamRect)
    : NFmiParamRect(theTextParamRect),
      ...
      itsMapping(theTextParamRect.itsMapping ? new NFmiParamMapping(*theTextParamRect.itsMapping)
                                             : 0),
...
int NFmiTextParamRect::ConvertDefText(NFmiString &object)
{
  ...
  if (lowChar == NFmiString("textalignment") || lowChar == NFmiString("alignment") ||
      lowChar == NFmiString("tekstikohdistus") || lowChar == NFmiString("kohdistus"))
    return dAlignment;
  ...
  else if (lowChar == NFmiString("size") || lowChar == NFmiString("textsize") ||
           lowChar == NFmiString("tekstikoko") || lowChar == NFmiString("koko"))
    return dParamSize;
  ...
  else if (lowChar == NFmiString("mapping") || lowChar == NFmiString("textmapping") ||
           lowChar == NFmiString("muunnos") || lowChar == NFmiString("tekstimuunnos"))
    return dTextMapping;

  else if (lowChar == NFmiString("rgbmapping") || lowChar == NFmiString("rgbmuunnos"))
    return dRGBMapping;
  ...
  else if (lowChar == NFmiString("addstationname") || lowChar == NFmiString("lis‰‰asemannimi"))
  ...
  else if (lowChar == NFmiString("fillwithunderscore") || lowChar == NFmiString("t‰yt‰alaviivalla"))
...

{
  ...
  {
    theDestinationFile << endl << "Font " << static_cast<char *>(GetFont()) << endl;
    ...
    theDestinationFile << "Text \"" << static_cast<char *>(locText) << "\" " << x << " " << y
                       << endl;
  }
  else
  {
    theDestinationFile << "%*** " << static_cast<char *>(theObjectName) << " STARTS ***" << endl;
   ...
   */
    ...
    {
      theDestinationFile << "/" << static_cast<char *>(GetFont()) << " /"
                         << static_cast<char *>(GetFont()) << "_" << endl;

      theDestinationFile << numberRect.Height() << " select" + GetEncoding() + "font" << endl;
    }

    else
    {
      theDestinationFile << "/" << static_cast<char *>(GetFont()) << " " << numberRect.Height()
                         << " selectfont" << endl;
    }

    bool firstIs226 =
        locText.GetLen() > 3 && NFmiString(locText.GetCharsPtr(1, 4)).IsEqual(longMinus);
    ...
    bool secondIs226 = locText.GetLen() > 4 &&
                       NFmiString(locText.GetCharsPtr(2, 4))
                           .IsEqual(longMinus);  // suluissa pitk‰ miinus tai numerov‰li
    ...
    /*
              if(!(firstIs226 || secondIs226)) //ei ongelmia
                    ...
                    {
                      if(boldFont)
                            //widthString = NFmiString("W"); //boldit v‰h‰n enemm‰n irti, levein
       kirjain
                            widthString = NFmiString("B"); //boldit v‰h‰n enemm‰n irti, levein
       kirjain
                      else
                            widthString = NFmiString("n"); //sama leveys kuin pitk‰ll‰ miinuksella
       kun kerran se itse ei k‰y stringwidth:iin
                      ...
                      if(locText.GetLen()>4)
                              ...
                              if(GetTextAlignment() == kLeft)
                              {
                                    WriteShowString(x, y, onlyMinus, restString,
       theDestinationFile);
                              }
                              else
                              {
                                    WriteShowString(x, y, restString, restString,
       theDestinationFile);
                              }
                    }
    */
                                        ...
                                        ofstream &os) const
{
  ...
  {
    os << "(" << static_cast<char *>(theShowString) << ") "
       << "stringwidth" << endl;
                                                 ...
                                                 ofstream &os) const
{
  ...
  if (GetTextAlignment() == kRight)
  {
    ...
    os << "(" << static_cast<char *>(totalWidth) << ") "
       << "stringwidth" << endl;
    os << "neg exch neg exch" << endl;
    os << " rmoveto" << endl;
  }
  else if (GetTextAlignment() == kCenter)
  {
    ...
    {
      os << "(" << static_cast<char *>(theWidthMinus) << ") "
         << "stringwidth" << endl;
      os << "-2. div exch -2. div exch" << endl;  // puolikas matka
      os << " rmoveto" << endl;
    }
