...
bool NFmiParamRect::ReadRemaining(void)
{
  ...
  switch (itsIntObject)
  {
    ...
    case dMultiParams:
    {
      ...
      if (itsMultiMapping)
      {
        *itsLogFile << "*** ERROR: Duplicate multimapping parameters rejected " << endl;
        break;
      }
    ...
    case dSum:
    {
      if (fModifierUsed)
      {
        *itsLogFile << "*** ERROR: Sum after a similar definition bypassed " << endl;
    ...
    case dWeightedMean:
    {
      ...
      else
      {
        if (!SetFive(long1,
                     long2,
                     itsIntegrationPeriod.startWeight,
                     itsIntegrationPeriod.centreWeight,
                     itsIntegrationPeriod.endWeight) ||
            long1 > long2)
    ...
    case dRelHour:
    {
      *itsLogFile << "*** ERROR: Not allowed for data elements: " << static_cast<char *>(itsString)
                  << endl;
      ReadNext();
      break;
    }
      ...
      if (ReadTwo(long1, long2))
      {
        *itsDescriptionFile >> itsObject;
        ...
        if (long1 > 0 && long2 > 0 && long3 > 0 && long1 <= long3)
        {
          ...
          *itsLogFile << "*** ERROR: Not valid station looping (start,step,stop): " << long1
                      << long2 << long3 << endl;
      }
...
int NFmiParamRect::ConvertDefText(NFmiString &object)
{
  ...
  if (lowChar == NFmiString("relplace") || lowChar == NFmiString("osaalue"))
    return dRelPlace;
  ...
  else if (lowChar == NFmiString("parameter") || lowChar == NFmiString("param") ||
  ...
  else if (lowChar == NFmiString("sum") || lowChar == NFmiString("summa"))
    return dSum;

  else if (lowChar == NFmiString("maximum") || lowChar == NFmiString("maksimi"))
    return dMaximum;
  ...
  else if (lowChar == NFmiString("minimum") || lowChar == NFmiString("minimi"))
    return dMinimum;
  ...
  else if (lowChar == NFmiString("areasum") || lowChar == NFmiString("aluesumma"))
    return dAreaSum;

  else if (lowChar == NFmiString("areamaximum") || lowChar == NFmiString("aluemaksimi"))
    return dAreaMaximum;

  else if (lowChar == NFmiString("areamean") || lowChar == NFmiString("aluekeskiarvo"))
    return dAreaMean;

  else if (lowChar == NFmiString("areaminimum") || lowChar == NFmiString("alueminimi"))
    return dAreaMinimum;

  else if (lowChar == NFmiString("yearperiod") || lowChar == NFmiString("vuosijakso"))
    return dYearPeriod;

  else if (lowChar == NFmiString("multiparams") || lowChar == NFmiString("multiparameters") ||
  ...
  else if (lowChar == NFmiString("degreedays") || lowChar == NFmiString("degreeday") ||
           lowChar == NFmiString("lämmitystarve") || lowChar == NFmiString("lämmitystarveluku"))
    return dDegreeDays;

  else if (lowChar == NFmiString("fahrenheit"))
    return dFahrenheit;

  else if (lowChar == NFmiString("putinstorage") || lowChar == NFmiString("laitavarastoon"))
    return dPutInStorage;

  else if (lowChar == NFmiString("usefromstorage") || lowChar == NFmiString("otavarastosta"))
  ...
  else if (lowChar == NFmiString("storagequeue") || lowChar == NFmiString("varastostojono"))
  ...
  else if (lowChar == NFmiString("probability") || lowChar == NFmiString("todennäköisyys"))
    return dProbability;

  else if (lowChar == NFmiString("allowmissing") || lowChar == NFmiString("sallipuuttuvia"))
    return dAllowMissing;

  else if (lowChar == NFmiString("acceptanceinterval") || lowChar == NFmiString("hyväksymisväli"))
    return dAcceptanceInterval;

  else if (lowChar == NFmiString("numbermapping") || lowChar == NFmiString("numeromuunnos"))
    return dInterval2Number;

  else if (lowChar == NFmiString("randomizationinterval") ||
           lowChar == NFmiString("randominterval") || lowChar == NFmiString("satunnaisväli"))
    return dRandomInterval;

  else if (lowChar == NFmiString("randomization") || lowChar == NFmiString("randommodifying") ||
  ...
  else if (lowChar == NFmiString("rounding") || lowChar == NFmiString("pyöristys"))
    return dRounding;
  ...
  else if (lowChar == NFmiString("missingvalue") || lowChar == NFmiString("puuttuvaarvo"))
  ...
  else if (lowChar == NFmiString("tempnotmean") || lowChar == NFmiString("lämpötilahetkellinen"))
    return dTempNotMean;

  else if (lowChar == NFmiString("rotate") || lowChar == NFmiString("käännä"))
    return dRotatingAngle;

  else if (lowChar == NFmiString("extremes") || lowChar == NFmiString("ääriarvot"))
    return dExtremePlotting;

  else if (lowChar == NFmiString("correctmaxtemp") || lowChar == NFmiString("korjaamaksimit"))
    return dTempMaxCorrection;

  else if (lowChar == NFmiString("correctmintemp") || lowChar == NFmiString("korjaaminimit"))
    return dTempMinCorrection;

  else if (lowChar == NFmiString("usebackuptime") || lowChar == NFmiString("käytävaraaikaa"))
    return dUseBackupTime;

  else if (lowChar == NFmiString("moonphase") || lowChar == NFmiString("kuunvaihe") ||
...
bool NFmiParamRect::PointOnParam(NFmiFastQueryInfo *theQI, NFmiParam *theParam)
{
  ...
  if (!theQI->Param(*theParam))
  {
    if (itsLogFile && !fParamErrorReported && !itsPressParam->HasPrimaryData())
    {
      ...
      *itsLogFile << "*** ERROR: Unable to find parameter: " << paramIdent << endl;
...
bool NFmiParamRect::PointOnMultiParam(NFmiFastQueryInfo *theQI, short theNum)
{
  ...
  if (!theQI->Param(NFmiParam(static_cast<unsigned long>(itsMultiParams[theNum]))))
  {
    if (itsLogFile && !fParamErrorReported && !itsPressParam->HasPrimaryData())
    {
      ...
      *itsLogFile << "*** ERROR: Unable to find multiparameter: " << paramIdent << endl;
      fParamErrorReported = true;
    }
    return false;
  }
...
bool NFmiParamRect::PointOnLevel(NFmiFastQueryInfo *theQI)
{
  ...
  if (!theQI->Level(itsLevel))
  {
    ...
    {
      *itsLogFile << "*** ERROR: Unable to find pressure level: " << itsLevel.LevelValue() << endl;
                                     ...
                                     bool localTimeSet)
{
  ...
  if (timeDiffNow < 30 &&
      itsEnvironment.UseBackupPreviousDay())  // 30 min before data begins to arrive
  {
  ...
  */
      ...
      itsPressParam->IsStationLocalTimeMode())
  {
    if (itsMultiMapping)
    {
      *itsLogFile << "    first station: local time= "
                  << static_cast<char *>(itsCurrentTime.ToStr("DD.MM.YYYY HH")) << " utc; par=multi"
                  << endl;
    }
    else
    {
    ...
    *itsLogFile
        << "      -> nearest data-time= "
        << static_cast<char *>(
               (static_cast<NFmiFastQueryInfo *>(theQueryInfo))->Time().ToStr("DD.MM.YYYY HH"))
        << " utc" << endl;
  }
  ...
  if (value != kFloatMissing && itsIdentPar == kFmiPresentWeather && value >= 100 && value < 200)
  {
...
bool NFmiParamRect::FloatValue(NFmiFastQueryInfo *theQueryInfo, float &value)
{
  ...
  if (actualModifier != kNoneModifier)
  {
    ...
    if (itsPressParam->IsFirstStation() && !(itsMultiMapping && itsCurrentMultiParNum > 1))
    {
      *itsLogFile << "      ->time period= "
                  ...
                  << static_cast<char *>(lastTime.ToStr("DD.MM.YYYY HH")) << " utc" << endl;
    }
    ...
    if (itsPressParam->IsStationLocalTimeMode())
    {
      ...
      if (IsMissingLonLat())
        *itsLogFile << "*** ERROR: longitude missing from "
                    ...
                    << " for calculating local time" << endl;
  ...
  bool varEtc =
      (fIsProbability || (itsAreaModifier != kNoneModifier && actualModifier != kNoneModifier) ||
  ...
  if (varEtc)
  {
    ...
    if (fIsProbability)  // **** sateen tn *** /
    {
      ...
      NFmiMaskedDataIterator geographicalArea(
          &ssinfo);  // comb/vai ei (ei tartte)  //tiealueet,vika aikajakso
      ...
      variationArea.SetDimensions(
          ssinfo.CalcAreaUnCertainty(), ssinfo.CalcAreaUnCertainty(), 0);  // aikavar. vika
      ...
      float areaRRProbMaxCalc =
          ssinfo.FloatValue(useVariation, useCalculator, !useTimeIntegration, !createVariance);
      value = areaRRProbMaxCalc;
    }
    // **** tiehallinnon sateen int epävarmuusalueella laajennettuna *** /
    else if (itsCurrentPar == kFmiPrecipitation1h && itsAreaModifier == kMaximum &&
             actualModifier == kNoneModifier)
    {
    ...
    else if (itsAreaModifier != kNoneModifier && actualModifier != kNoneModifier)
    {
      ...
      NFmiRelativeTimeIntegrationIterator timeIterator(
          &ssinfo, itsLastExtremRelHour - itsFirstExtremRelHour + 1, -itsLastExtremRelHour);
  ...
  else
  {
    if (theQueryInfo->IsGrid())
    {
      if (isWeightedMean ||  // ajan suhteen
          (period > 0 &&
           (par == kFmiWeatherSymbol1 || par == kFmiWeatherAndCloudiness ||
            par == kFmiTotalWindMS || par == kFmiMiddleAndLowCloudCover ||
            par == kFmiPrecipitationForm || par == kFmiPrecipitation1h || par == 375) &&
           (actualModifier == kNoneModifier || actualModifier == kMean)))
      {
        value = static_cast<float>(theQueryInfo->InterpolatedTimePeriodFloatValue(
            itsPressParam->GetCurrentStation().GetLocation(),
            firstTime,
            lastTime,
            startWeight,
            centreWeight,
      ...
      else  // ei painotettu aikakeskiarvo
      {
        if (actualModifier != kNoneModifier)
        {
          ...
          {
            // pitää olla modif
            NFmiDataModifierWindChill wcMod(
                theQueryInfo, modif, itsPressParam->GetCurrentStation().GetLocation());
          ...
          else
          {
            if (itsPressParam->IsAreaOperation())
            {
              ...
              (static_cast<NFmiSuperSmartInfo *>(theQueryInfo))
                  ->AreaMask(itsPressParam->GetAreaMask());
              (static_cast<NFmiSuperSmartInfo *>(theQueryInfo))->UseAreaMask(true);
            }
            theQueryInfo->CalcInterpolatedTimeData(
                modif, firstTime, lastTime, itsPressParam->GetCurrentStation().GetLocation());
            if (isExtremeModifier)
              itsPressParam->SetOptionTime(
                  (static_cast<NFmiDataModifierExtreme *>(modif))->GetTime());
            value = modif->CalculationResult();
          }
        }
        else if (par == kFmiChillFactor)
        {
          NFmiDataModifierWindChill wcMod(
              theQueryInfo, modif, itsPressParam->GetCurrentStation().GetLocation());
          theQueryInfo->CalcTimeData(&wcMod, firstTime, lastTime);
        ...
        else
        {
          ...
          else if (itsPressParam->IsAreaOperation())
          {
            ...
            {
              *itsLogFile << "*** ERROR: "
                          << "Modifier missing from areaoperation (sum,max...)" << endl;
              value = kFloatMissing;
            }
          }
          else
            ...
            value =
                theQueryInfo->InterpolatedValue(itsPressParam->GetCurrentStation().GetLocation());
        }
      }
    }
    else  // ei gridi
    {
      /***** HUOM 9.12.00: pitäisi varmaan estää modifierin käyttö aikakeskiarvon kanssa
                     kuten gridissä tehtiin (esim olomuoto tulee flotarina ja muutenkin se kai olisi
         se oikea tapa)
      */
      if (par == kFmiChillFactor)
      {
      ...
      else
      {
        if (actualModifier != kNoneModifier)
        {
          if (isExtremeModifier)
          {
            theQueryInfo->CalcTimeDataWithExtremeTime(
                static_cast<NFmiDataModifierExtreme *>(modif), firstTime, lastTime);
            itsPressParam->SetOptionTime(
                (static_cast<NFmiDataModifierExtreme *>(modif))->GetTime());
          }
          else
        ...
        {
          theQueryInfo->CalcLocationDataWithExtremePlace(
              static_cast<NFmiDataModifierExtremePlace *>(placeModif));
  ...
  /* näin pitäisi olla mutta vaatisi muutoksia tiehallintoon
    if(fUseFromStorageConditionally)
    {
          ...
          {
            if(storedValue != kFloatMissing && storedValue < 105) //HUOM niinkauan kun pohjoisin
    suomi puuttuu tutkasateista
                  {
                    value = storedValue;
...
*/
  ...
  // tämä taas toimii ok tiehallinnon kanssa mutta tuottaa turhia varoituksi käyttämättömistä
  // varastoarvoista
  if (fUseFromStorageConditionally &&
      value == kFloatMissing)  // 4.10.01 olkoon ainoa ehto nyt aluksi että arvo puuttuu
  {
    float storedValue = itsPressParam->UseFromStorage(itsStorageQueue);
    if (storedValue != kFloatMissing &&
        storedValue < 105)  // HUOM niinkauan kun pohjoisin suomi puuttuu tutkasateista
    {
      value = storedValue;
      *itsLogFile << "   * INFO: "
                  << "stored value used instead of actual" << endl;
    }
  }
      ...
      itsPressParam->GetPressProduct()->GetSupplementMode() && !(itsPressParam->IsSupplementary()))
  {
    ...
    if (itsPressParam->IsFirstStation())
      *itsLogFile << "   * INFO: "
                  << "missing value to be supplemented" << endl;
  }

  if (itsInterval2NumberMin != kFloatMissing)
    value = itsInterval2NumberMin <= value && value <= itsInterval2NumberMax
                ? itsInterval2NumberValue
                : value;

  if (itsValueIntervalMin != kFloatMissing &&
      (!(itsInterval2NumberMin != kFloatMissing && value == itsInterval2NumberValue)))
  {
  ...
  {
    GetPressProduct()->GetTempCorrection()->CorrectValue(
        value, itsPressParam->GetCurrentStation().GetName(), fTempMaxCorrection);
  }
  ...
  if (IsRandom())
  {
    ...
    value += static_cast<float>(GetRandomInterval()) *
             static_cast<float>((static_cast<float>(rand()) / RAND_MAX - 0.5));
...
float NFmiParamRect::SunElevation(NFmiFastQueryInfo *theQueryInfo)
{
  ...
  return static_cast<float>(
      location.ElevationAngle((static_cast<NFmiFastQueryInfo *>(theQueryInfo))->Time()));
}
...
NFmiMetTime NFmiParamRect::CalculatePeriodTime(long theHour)
{
  ...
  if (itsPressParam->IsStationLocalTimeMode())
  {
    ...
    if (IsMissingLonLat())
      *itsLogFile << "*** ERROR: Longitude missing from "
                  << static_cast<char *>(itsPressParam->GetCurrentStation().GetName())
                  << " for calculating local time (Calc.Per.Time)" << endl;
  ...
  for (int i = 0; static_cast<int>(itsMultiParams[i]) != static_cast<int>(kFmiLastParameter); i++)
  {
        ...
        value == kFloatMissing)
    {
      ...
      *itsLogFile << "  *** WARNING: " << enumConv.ToString(itsMultiParams[i])
                  << " missing for time "
                  << static_cast<char *>(theQI->Time().ToStr("DD.MM.YYYY HH"))
...
bool NFmiParamRect::RandomizeRelatively(float &theValue,
                                        float theInterval,
                                        float theMin,
                                        float theMax) const
{
  ...
  if (itsPressParam->IsStationLocalTimeMode())
  {
    ...
    if (IsMissingLonLat())
    {
      *itsLogFile << "*** ERROR: Longitude missing for "
                  << static_cast<char *>(itsPressParam->GetCurrentStation().GetName())
                  << " for calculating local time (SetStat.Loc.Time)" << endl;
      longitude = 0.;
    }
    else
...
bool NFmiParamRect::SetRelativeHour(NFmiFastQueryInfo *data, const NFmiString &calledFrom)
{
  if (RelativeHour() != 0)
  {
    ...
    {
      if (itsPressParam->IsStationLocalTimeMode())
        *itsLogFile << "    hour changed in " << static_cast<char *>(calledFrom) << ": "
                    << itsFirstPlotHours << " local time" << endl;
      else
        *itsLogFile << "    hour changed in " << static_cast<char *>(calledFrom) << ": "
                    << itsFirstPlotHours << " utc" << endl;
    }
  }
  return true;
}
