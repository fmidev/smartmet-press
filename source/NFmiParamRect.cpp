...
// Muutettu 291297/LW Producer ja param nyt myˆs yhdell‰ rivill‰
...
// Muutettu 190899/LW +itsCombinedIdent eli parametri voi olla TotalWindiss‰ tai WeatherAndCluodiness:ssa
// Muutettu 190899/LW TUOTTAJA POISTETTU DATAN HAUSSA, pelkk‰ parametri j‰‰, koko dataIdentti edelleen j‰sen
// Muutettu 020999/LW +dWeightedMean
// Muutettu 230999/LW multiParamMapping ja kaikki mik‰ siihen kuuluu:PointOnMultiParam() jne
// Muutettu 290200/LW copy-konstr. siivottu
// Muutettu 280300/LW auringonKorkeuden k‰sittely
 ...
 ,itsDataIdent(theRect.itsDataIdent)
{
 ...
 itsLogFile  = theRect.GetLogFile();   //n‰m‰ perityt j‰senet kun eiv‰t ole emon copy-
 itsMaxLoopNum = theRect.GetMaxLoopNum(); //konstruktorissa niin olkoon t‰ss‰
...
//----------------------------------------------------------------------------
bool NFmiParamRect::ReadRemaining(void)  //090299
{
//	float float1, float2;
	long long1, long2;
	...
	switch(itsIntObject)
	{
		...
		case dMultiParams:	  
		{
			...
			if(itsMultiMapping)
			{
			   *itsLogFile << "*** ERROR: MoniMuunnoksen parametrit toiseen kertaan, hyl‰t‰‰n " << endl;
				break;
			}
			...
			itsString = itsObject; //Huom uusi tapa k‰ytt‰‰ ReadLongia (stringi joka lopettaa ja samalla on seuraava) 
		...
		case dSum:	//9.5.2000  
		{
			if(!SetTwo(long1, long2)
				|| long1 > long2)
					*itsLogFile << "*** ERROR: Summa-ajat v‰‰rin " << endl;
			else
			{
		...
		case dMinimum:	//3.2.2000  
		{
			if(!SetTwo(long1, long2)
				|| long1 > long2)
					*itsLogFile << "*** ERROR: Minimiajat v‰‰rin " << endl;
			else
			{
				SetRelModifierTimes(long1, long2);
		...
		case dMaximum:	//3.2.2000  
		{
			if(!SetTwo(long1, long2)
				|| long1 > long2)
					*itsLogFile << "*** ERROR: Maksimiajat v‰‰rin " << endl;
			else
			{
				SetRelModifierTimes(long1, long2);
		...
		case dMean:	//3.2.2000  
		{
			if(!SetTwo(long1, long2)
				|| long1 > long2)
			{
				*itsLogFile << "*** ERROR: Keskiarvoajat v‰‰rin " << endl;
			}
			else
			{
				SetRelModifierTimes(long1, long2);
		...
		case dWeightedMean:	//3.2.2000  
		{
			if(!SetFive(long1, long2
			   ,itsIntegrationPeriod.startWeight,itsIntegrationPeriod.centreWeight,itsIntegrationPeriod.endWeight)
				|| long1 > long2)
					*itsLogFile << "*** ERROR: Painotettu keskiarvo annettu v‰‰rin " << endl;
			else
			{
				SetRelModifierTimes(long1, long2);
...
//			itsIdentPar = kFmiTemperature; EI, pit‰‰ muistakin l‰mmˆist‰ saada Fahr.(oletus 4)
		...
		case dYearPeriod:	//28.11.2000  
		{
			if(!SetTwo(long1, long2) 
			   || long1 > long2	)
			{
				*itsLogFile << "*** ERROR: Vuosijakso annettu v‰‰rin " << endl;
			}
			else
			{
...
//3.6.02 voidaan k‰ytt‰‰ managerin kanssa myˆs ---------------------------------------------------------------------------
bool NFmiParamRect::UpdateModifierTimes(void)
{
//	itsFirstPlotHours = itsFirstPlotTime.GetHour(); //4.6.02 EI KƒY
...
int NFmiParamRect::ConvertDefText(NFmiString & object) 
{
	...
	if(lowChar==NFmiString("¥relplace") ||            lowChar==NFmiString("osaalue")) //4.2
		...
		return dMinimum;
		                                    ...
		                                    ||        lowChar==NFmiString("alueyhdistelm‰"))//24.9.01
		return dAreaUnion;
		...
		return dWindCill;
	else if(lowChar==NFmiString("degreeday")||     lowChar==NFmiString("l‰mmitystarve")
	...
	else if(lowChar==NFmiString("probability")||     lowChar==NFmiString("todenn‰kˆisyys"))  //18.9.01
		return dProbability;
	else if(lowChar==NFmiString("allowmissing")||     lowChar==NFmiString("sallipuuttuvia"))  //21.9.01
		return dAllowMissing;
	else if(lowChar==NFmiString("acceptanceinterval")||     lowChar==NFmiString("hyv‰ksymisv‰li"))  //2.11.01
...
{	//2.5.00 yhdistetty PointOnSecondParam t‰h‰n
	//27.3.00 jos kysyt‰‰n auringon korkeutta se lasketaan eik‰ haeta datasta
...
//	if (!theQI->Param(*GetDataIdent().GetParam())) //19.8.99 TUOTTAJA POISTETTU
	...
	// 19.8.99
    // jos ei pelkk‰n‰ parametrina niin lˆytyisikˆ CombinedParametrina?
...
//			   long paramIdent = GetDataIdent().GetParam()->GetIdent();
			   ...
			   *itsLogFile << "  *** ERROR: Parametria ei lˆydy: " << paramIdent 
						/*<< "/" << prodIdent*/ << endl;
...
{ 
	//28.3.00 jos kysyt‰‰n auringon korkeutta se lasketaan eik‰ haeta datasta
...
//			   long paramIdent = GetDataIdent().GetParam()->GetIdent();
			   ...
			   *itsLogFile << "  *** ERROR: multiParametria ei lˆydy: " << paramIdent << endl;
			   fParamErrorReported = true;
			}
			return false;
		}
...
//		{
			...
			{
			   *itsLogFile << "*** ERROR: Painepintaa ei lˆydy: " << itsLevel.LevelValue() << endl;
...
bool NFmiParamRect:: ReadCurrentValue(NFmiFastQueryInfo* theQueryInfo, float& value, bool localTimeSet)  
{

//	NFmiMetTime saveTime = theQueryInfo->Time(); //16.2.2000 pois, eikˆh‰n alkuasetus groupissa riit‰
	if(!localTimeSet)                 //multimappingiss‰ jo asetettu
	{
		 if(!SetStationLocalTime(theQueryInfo)) //1.2.2000 v‰liaikaisesti jos optio p‰‰ll‰
...
//	16.2.2000 ****** muualla ei en‰‰ kosketa QInfon aikaan
	if(!theQueryInfo->TimeToNearestStep (itsCurrentTime, kCenter, theQueryInfo->TimeResolution()/2))
	{
		if(!fTimeErrorReported) //3.5.02 t‰nne joudutaan jos piirtoelementiss‰ muutetaan olemattomaan aikaan
		{
		...
		&& itsPressParam->IsStationLocalTimeMode())
	{
		if(itsMultiMapping)
		{
			*itsLogFile << "    eka asema: aika paikan p‰‰ll‰= " << itsCurrentTime << " utc; par=moni" << endl;
		}
		else
		{
			unsigned long par = theQueryInfo->ParamDescriptor().Param().GetParam()->GetIdent();
			*itsLogFile << "    eka asema: aika paikan p‰‰ll‰= " << itsCurrentTime << " utc; par=" << par << endl;
		}
		*itsLogFile << "      ->l‰hin data-aika= " << (static_cast<NFmiFastQueryInfo *>(theQueryInfo))->Time() << " utc" << endl;
	}
...
{ //WaWa koodia varten l‰mpˆtila
	...
	{
	  *itsLogFile << "  WARNING: L‰mpˆ‰ ei ole WaWalle" << endl;
	  return value;
	}
...
{    //1.11.99 piti erottaa jotta k‰‰nt‰v‰arvo ei tarttis k‰ytt‰‰ itsValueFactor:ia
	// 2.2000 qInfon CalcTimeData(modif..) k‰yttˆˆn
...
*/
	...
	if(itsCurrentPar == kFmiSolarElevation) //28.3.00 auringon korkeus metodilla eik‰ datasta 
	{
...
//	NFmiMetTime firstTime = ((NFmiFastQueryInfo*)theQueryInfo)->Time(); //4.9.01
	...
	bool isWeightedMean = period > 0 && period != kUnsignedLongMissing || itsModifier == kWeightedMean;
	...
	if(itsModifier != kNoneModifier )  //4.2.2000
	{
		...
		if(itsPressParam->IsFirstStation() && !(itsMultiMapping && itsCurrentMultiParNum > 1))
		{
//		    unsigned long ownPar = GetDataIdent().GetParam()->GetIdent(); //ei hyv‰
...
// 22.2.2000 HUOM: OLETUS ON HETKELLINEN EIKƒ JAKSO 
...
//         ===============
	...
	//		float areaRRProbMeanCalc = ssinfo.FloatValue(useVariation, useCalculator, !createVariance);
		...
		else if(itsCurrentPar == 353 && itsAreaModifier == kMaximum && itsModifier == kNoneModifier) // || itsAreaModifier == kUnion   eik‰ todenn.) 
//         ===============
        {
//			ssinfo.AreaUnCertaintyStart(0);//0-> 0 tai 100
//			ssinfo.AreaUnCertaintyEnd(0);//N‰m‰ ovat vain kokeellisia arvoja, pit‰isi tulla oikeasti datan mukana meteorologilta
...
/*			
			    ...
			    newMask.Mask(3, true);
			    newMask.Mask(4, true);
			    newMask.Mask(31, true);
			    newMask.Mask(32, false); //HUOM
			    newMask.Mask(33, true);
			    newMask.Mask(34, true);
				bool test1 = newMask.IsMasked(1215);
				bool test2 = newMask.IsMasked(1218);
	   			theQueryInfo->StaticDataMask(newMask, true);
//				theQueryInfo->UseStaticMask(true);
...
//sis‰‰n vrk:n keskil‰mpˆtila
// huom eri kynnysarvo alku/loppuvuodelle (s‰teilyn vaikutus)
{
	if(value == kFloatMissing)
...
//		tim.ChangeByDays(1);
//		hour -= 24;
		long days = theHour/24; //9.5.00 aikasarjat ulotettu yli yhden vuorokauden siirtymist‰
...
*/
	...
	if(itsPressParam->IsStationLocalTimeMode())
	{
		double longitude = itsPressParam->GetCurrentStation().GetLongitude();
		double latitude = itsPressParam->GetCurrentStation().GetLatitude();//Testiin
		if(fabs(longitude) <= .001 && fabs(latitude) <= .001)   //24.5.00 abs lis‰tty
		  *itsLogFile << "*** ERROR: longitudi puuttuu " << static_cast<char *>(itsPressParam->GetCurrentStation().GetName())
					<< ":lta paikallista aikaa varten"<< endl; 
...
{
//	NFmiCombinedParam* combinedParam = 0; //Yritet‰‰n ottaa kaikki parametrit t‰st‰
	                                      // kun kerran paljon aikaa on mennyt sen laskemiseen

	if(!SetStationLocalTime(theQI)) //1.2.2000 v‰liaikaisesti jos optio p‰‰ll‰
	{
		return false;
	}
...
//	bool singleTime = itsIntegrationPeriod.period == 0 || itsIntegrationPeriod.period == kLongMissing;
    ...
    //Yritet‰‰n ottaa kaikki parametrit kerran lasketusta combinedParamista
	// kun kerran paljon aikaa on mennyt sen kostruoimiseen
	// ei kannata k‰ytt‰‰ ristiin tuuli ja weatherAndCloudiness-parametreja monimuunnos-m‰‰rittelyss‰ 
...
//			&& !singleTime)   //29.5.00 jos yksitt‰ist‰ aikaa QD:ss‰ ei v‰ltt. combinedPar
//		{
...
//		{
	...
	// jos signifikantti puuttuu multiMapping ilmoittaa
			...
			ReadCurrentValue(theQI, value, true); //15.2.00 =local time jo asetettu
...
{  //optio
	if(itsPressParam->IsStationLocalTimeMode())  
	{
      //HUOM sama koodi yll‰pisett‰v‰ NFmiTimeParamRect:ss‰
		double longitude = itsPressParam->GetCurrentStation().GetLongitude();
		double latitude = itsPressParam->GetCurrentStation().GetLatitude();//Testiin
...
// 16.2.2000 t‰ll‰ hoidetaan se ett‰ alle vuorokaudenkin aikaluupit onnistuvat alkioiden omilla tunneilla
{
   if(RelativeHour() != 0)
   {
...
*/
/* ei kun writePS j‰seneksi ja sielt‰ k‰yttˆˆn
//---------------------------------------------------------------------------
//24.10.00 tuli myˆs WritePS:‰‰n !!!
