...
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
FmiBoolean NFmiPressTimeText::ReadDescription(NFmiString& retString) //16.1
	...
	switch(itsIntObject)
	{
		...
		case dSymbolPlace:
		{
			...
			if(Read2Double(r1,r2))
			{
			   ...
			   itsRotatingPoint = NFmiPoint(r1,r2); //riittää kun antaa joko Paikan tai RotPaikan
			}
			ReadNext();
			break;
		}
...
/*		case dTextColor:
		...
		*/
		...
		case dTimeTextFormat:	//1.3.2000 lisäyksiä	  
		{
			if (!ReadEqualChar())
				break;
