...
NFmiPressImage::NFmiPressImage(void)
    : NFmiPressScaling(), itsPressProduct(0), itsRelHoursFromFirst(0), itsShear(0.)
{
  ...
  *itsDescriptionFile >> itsObject;
  ...
  else
  {
    ...
    if (env == 0)
      ...
      itsPath = CreatePath(
          NFmiString(env), itsTempImagePath, itsTempImageDir, itsTempImageFile, defExtension);
#else
    ...
    itsPath = CreatePath(dir, itsTempImagePath, itsTempImageDir, itsTempImageFile, defExtension);
...
int NFmiPressImage::ConvertDefText(NFmiString& object)
{
  ...
  if (lowChar == NFmiString("file") || lowChar == NFmiString("tiedosto") ||
  ...
  else if (lowChar == NFmiString("summerwinterchoice") || lowChar == NFmiString("kes‰talvivalinta"))
    return dSummerWinterChoise;
           ...
           lowChar == NFmiString("kes‰talvitiedosto") || lowChar == NFmiString("kes‰talvinimi"))
    return dSummerWinterImageFile;

  else if (lowChar == NFmiString("newimage") || lowChar == NFmiString("uusikuva"))
    return dNewImage;

  else if (lowChar == NFmiString("newname") || lowChar == NFmiString("uusitiedosto") ||
  ...
  else if (lowChar == NFmiString("newfilemoving") || lowChar == NFmiString("newrelatively") ||
           lowChar == NFmiString("uusitiedostosiirtyen") ||
           lowChar == NFmiString("uusisuhteellisena") || lowChar == NFmiString("uusisuht"))
    return dNewImageRel;

  else if (lowChar == NFmiString("directory") || lowChar == NFmiString("hakemisto"))
    return dImageDir;

  else if (lowChar == NFmiString("rotationfirst") || lowChar == NFmiString("rotatefirst") ||
  ...
  else if (lowChar == NFmiString("currentscalingpoints") || lowChar == NFmiString("plottingview") ||
           lowChar == NFmiString("nykyskaalauspisteet") || lowChar == NFmiString("sijoitusalue"))
  ...
  else if (lowChar == NFmiString("initialscalingpoints") || lowChar == NFmiString("mapdefsize") ||
           lowChar == NFmiString("alkuskaalauspisteet") || lowChar == NFmiString("mittaalue"))
    return dImageDefSize;

  else if (lowChar == NFmiString("clippingrectangle") || lowChar == NFmiString("clip") ||
           lowChar == NFmiString("rajaus"))
    return dImageClippingRectangle;

  else if (lowChar == NFmiString("clippingpath") || lowChar == NFmiString("rajauspisteet"))
    return dImageClippingPath;

  else if (lowChar == NFmiString("shear") || lowChar == NFmiString("kallistus"))
    return dImageShear;
...
// bool precedingElementMissing = false;
  ...
  {
    *itsLogFile << "WARNING: image marked None" << endl;
    return true;
  }
...
//				  +static_cast<char *>(theCountryPart));
  ...
  else
  {
    ...
    {
      NFmiCopyFileCroppingWithoutShowpage(
          itsInFile, itsOutFile, NFmiRect(NFmiPoint(0., 0.), NFmiPoint(0., 0.)));
    }
    else
...
#else
    *itsLogFile << "EPS image included into eps output: " << static_cast<char*>(imageFile) << endl;
#endif
  }
  itsInFile->close();
